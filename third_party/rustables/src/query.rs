    ...
    QueryError: From<E>,
{
    ...
    // NPorter patch (vs upstream 0.8.7): do NOT close `sock` here.
    // With nix 0.30, `socket::socket()` returns an `OwnedFd` that the caller
    // still holds and which closes itself on drop. Closing the raw fd here as
    // well caused a double-close, which aborts under Rust 1.95's IO-safety
    // checks ("owned file descriptor already closed"). Letting the OwnedFd drop
