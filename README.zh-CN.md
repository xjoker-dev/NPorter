...
- 可选 UFW 集成：为启用的 mapping 自动同步带 NPorter 标识的 route allow 规则。
...
- 默认不管理 UFW；启用 `[ufw].manage` 时需要安装 `ufw`。
...
UFW 默认 `deny (routed)` 时会丢弃已完成 DNAT 的转发流量。可让 NPorter 在每次
`apply` 时同步所需的 UFW route allow 规则：
...
NPorter 只增删 comment 为 `NPorter:<hash>` 的 route 规则，不修改 UFW 默认策略或其他
规则。多个 mapping 指向同一协议、目标 IP 和目标端口时共享一条 UFW 规则。

将 `ufw.manage` 设为 `false` 会停止同步，但不会删除现有规则。如需清理，先在管理开启
时禁用或删除 mappings 并执行 `apply`，再关闭管理。
...
- 当 `ufw.manage = true` 时，`apply` 会同步 NPorter 自有的 UFW route allow 规则。
